...
class Ewey extends MY_Controller {
	
	public function __construct()
	{
		parent::__construct();
		//define('online_Site_Url','http://www.binaryfrog.co/delta/#/');
		define('online_Site_Url','http://www.fruitastic.com.au/#/');
	...
	public function index()
	{
		...
		if (isset($_POST['btnSubmit'])) 
		{
			...
			*/
			...
			$self_url=base_url('payment/ewey/return_url');
			
			$request->RedirectUrl = $self_url;
			$request->CancelUrl   = base_url('payment/ewey/cancle_url');
	...
	function loader_function()
	{
		...
		$t=0;
		/*	do {
				echo "The number is: ".date('d-m-y  H:i:s',strtotime($x))." <br>";
				$x=time();
			} while ($x <= $xt);		
		for($i=$x;$i<$xt;$i++)		//9999999999	
		{
			if($i>$t)
			{
				echo $i.'<br/>';//date('H:i:s',strtotime($i));
				$t=time()+1;
			}
			$i=time();
		}*/
