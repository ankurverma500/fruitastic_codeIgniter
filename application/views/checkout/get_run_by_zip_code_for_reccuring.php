<?php 
//echo '<pre>';
//$rec_order=get_order_run_detail($order_id,$customer_id,'recurring');
$rec_order=get_order_run_detail_by_session($order_id,$this->added_by,'recurring');
...
for($i=1;$i<$counter;$i++)
{
	...
	echo ','.$current_day_name=date('l',strtotime( $newdate ));
			...
			<div class="col-sm-4" style="height: auto; margin-left: 0px !important;">
				<div class="checkbox" style="padding-bottom: 0px !important; ">
				  <input id="<?php echo $current_day_name.'_'.$newdate;?>" run_date="<?php echo $newdate;?>" run_day="<?php echo $current_day_name;?>" type="checkbox" <?php echo $checked;?> class="day_name_hedding">
				  ...
				  <div class="radio" style="margin-left: 20px;">
					<input type="radio" <?php echo $checked;?> class="day_name_class_recuring" run_detail_id="<?php echo $mn->run_detail_id?>" name="<?php echo $current_day_name?>" tbl_run_id="<?php echo $mn->tbl_run_id?>" id="<?php echo $current_day_name.'_'.$mn->run_detail_id?>" run_date="<?php echo $mn->run_date;?>"  run_day="<?php echo $current_day_name;?>" />
	...
	{
	echo 	'not run in this Zip code '.$i;
	}*/
...
$(".day_name_class_recuring").on('click',function(){
	//var perent_div_id=
	//var order_run_type=$('.delivery_day_radio').prop('checked',true).val();
	var order_run_type=$(".delivery_day_radio:checked").val();
	//alert(order_run_type);
	...
	var data_array={order_id:order_id,customer_id:customer_id,run_detail_id:run_detail_id,run_day_name:run_day,run_date:run_date,tbl_run_id:tbl_run_id,order_run_type:order_run_type};
	send_ajax1(send_url,data_array);
});

			
$(".day_name_hedding").on('click',function(){
	var order_run_type=$(".delivery_day_radio:checked").val();
	var run_day=$(this).attr('run_day');
	var run_date=$(this).attr('run_date');
	if($(this).is(":checked")) 
	...
	else
	{
		...
		if($('.day_name_hedding:checkbox').length<1)
		{
		...
		$(this).parent('div .checkbox').siblings('div .radio').children('.day_name_class_recuring').prop("checked",false);
	}
});
